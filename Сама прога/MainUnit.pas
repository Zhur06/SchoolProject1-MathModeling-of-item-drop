...
uses ChildUnit, IniFiles, ChildUnit2, ChildUnit3, ChildUnit4, ChildUnit5;
...
//----------------- Считывание данных для третьего графика ---------------------
...
//----------------- Сохранение данных третьего графика в файл ------------------

procedure TMainForm.Button8Click(Sender: TObject);
var ini: TIniFile; strH, strG: string; c: integer;
begin
  strH := Edit4.Text;                                                           //Проверка что в поле для ввода высоты пользователь ввел число
  ...
  strG := Edit5.Text;                                                           //Проверка что в поле для ввода ускорения пользователь ввел число
...
begin
  if Edit1.Text = 'Введите h' then Edit1.Text := ''
...
begin
  ...
  if numberic then Edit1.Text := 'Введите h';
end;
...
begin
  if Edit4.Text = 'Введите h' then Edit4.Text := ''
...
begin
  ...
  For i := 0 to length(Edit4.Text) - 1 do
    if i in [0..9] then numberic := false;
  if numberic then Edit4.Text := 'Введите h';
end;
...
begin
  if Edit5.Text = 'Введите g' then Edit5.Text := ''
...
begin
  ...
  Max1 := Canvas.TextWidth('Считать данные из ячеек') + 4;
  Max2 := Canvas.TextHeight('Считать данные из ячеек') + 4;
...
//----------------- Считывание данных для пятого рисунка -----------------------
